...
const logWork = (data: any, req: any, input: boolean) => {
  if (input) {
    logger.info(
      { pid: process.pid + ", " + cluster.worker?.id, data: data },
      `Take ${req.method} ${req.params.module}/${req.params.action}`
    );
  } else {
    logger.info(
      { pid: process.pid + ", " + cluster.worker?.id, data: data },
...
APP.use("/api/:module/:action", async (req, res, next) => {
  if (cluster.isWorker) {
    ...
    if (modules[moduleName] && modules[moduleName][actionName]) {
      ...
      logger.error(
        { pid: process.pid + ", " + cluster.worker?.id },
        `No api url /${moduleName}/${actionName}`
      );
      return res
        .status(404)
        .json({
...
// настройка рабочего
  ...
  APP.listen(PORT, () =>
    logger.info(
      `Worker ${cluster.worker?.id} launched, pid: ${process.pid}, port: ${PORT}`
    )
  );
